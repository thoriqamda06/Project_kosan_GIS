...
<body class=" ">
        ...
        <div class="bg-gray-200 p-8 rounded-xl">
            ...
            <script>
                ...
                cariButton.addEventListener('click', () => {
                    ...
                    allKosanListContainer.classList.add('hidden');
            ...
            <div id="all-kosan-list" class="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 gap-4"></div>
        </div>
        <!-- Judul -->
    ...
    <script>
        const allKosanListContainer = document.getElementById('all-kosan-list');

        // Fetch semua data kosan dari server
        const fetchAllKosan = () => {
            fetch('../data_kos.php', {
                method: 'POST',
                headers: {
                ...
                .then(response => response.json())
                .then(data => {
                    allKosanListContainer.innerHTML = '';
                    ...
                    if (data.length > 0) {
                        ...
                        allKosanListContainer.innerHTML = '<p class="col-span-full text-center text-gray-500">Tidak ada kosan tersedia saat ini.</p>';
                    }
                })
                .catch(error => {
                    console.error('Error:', error);
                    allKosanListContainer.innerHTML = '<p class="col-span-full text-center text-red-500">Terjadi kesalahan saat memuat data kosan.</p>';
                });
        };

        // Panggil fetchAllKosan saat halaman dimuat
        fetchAllKosan();
    </script>
